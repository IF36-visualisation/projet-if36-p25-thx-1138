...
ggplot(medal_type_long_plot, aes(x = Medal_Type, y = Average_Count, fill = Host_Status)) +
  ...
  scale_fill_manual(values = c("Pays Hôte" = "#69b3a2", "Pays Non-Hôte" = "#404080")) + # Couleurs plus distinctes
  labs(
...
ggplot(host_medal_trend_extended, aes(x = Phase, y = Medals, group = 1)) + # group = 1 car chaque facette a une seule ligne
  geom_point(size = 3, color = "steelblue") + # Couleur fixe pour les points dans chaque facette
  geom_line(linewidth = 1, color = "steelblue") + # Couleur fixe pour les lignes dans chaque facette
  labs(
...
ggplot(long_term_impact_pivot, aes(x = Host_Event_Label, y = Average_Medals, fill = Period)) +
  ...
  scale_fill_manual(values = c("Moyenne Avant Hôte" = "#C0C0C0", "Moyenne Après Hôte" = "#FFD700")) + # Couleurs plus distinctes
  theme_minimal(base_size = 12) +
  theme(
...
Pour tester ces hypothèses, nous allons visualiser l’évolution de la taille moyenne et du poids moyen des athlètes par discipline et par année, à l’aide de graphiques line chart. Ces visualisations permettront d’identifier les tendances globales ainsi que les différences entre sports.
